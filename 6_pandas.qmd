...
## MultiIndex Construction Methods (3/3)
...
Convert multi-indexed **Series** to **DataFrame** `.unstack()`
...
## Data Aggregations - multi-indexed
...
`pd.concat([df1, df2])` Default: Concatenate vertically (`axis=0`)

`df1` and `df2` can be any DataFrames & input should be DataFrame list
...
You can concatenate column-wise by `axis=1` in `pd.concat()`
...
1.  **Verify integrity**: Raise an error if duplicates exist `verify_integrity=True` in `pd.concat()`
...
2.  **Ignore indices**: Create new integer indices `ignore_index=True` in `pd.concat()`
...
3.  **Add hierarchical keys**: Create a MultiIndex to distinguish sources `keys=` in `pd.concat()`
...
When concatenating DataFrames with shared columns - match index!
...
Using the common column as the key. `pd.merge(df1, df2)`. `df1` and `df2` can be any DataFrames
...
Using the `on` parameter in `pd.merge()`
...
`left_on` and `right_on` parameter in `pd.merge()`
...
The DataFrame `df1.join()` method provides a shortcut for **index-based** merges. `df1` can be any DataFrame
...
Custom suffixes with `suffixes` parameter in `pd.merge()`
...
`df.mean()`, `df` can be any DataFrame
...
Use axis parameter `axis='columns'` in aggregation function `df.mean()` to aggregate across rows instead (**collapse columns**), `df` can be any DataFrame
```{python}
#| echo: true
print(df)
```
...
quick statistical summary of your data: `df.describe()`, `df` can be any DataFrame
...
`df.groupby()` for more sophisticated data analysis (`df` can be any DataFrame) by:
...
## GroupBy: Split, Apply, Combine
`df.groupby(by?).aggregation_fun()`, `df` can be any DataFrame
```{python}
#| echo: true
...
**Aggregation**: Multiple aggregations at once. `df.groupby().aggregate(function list)`, `df` can be any DataFrame
...
Different aggregations per column `df.groupby().aggregate(function dict)`, `df` can be any DataFrame
...
`df.groupby().filter(boolen or function)`, `df` can be any DataFrame
...
-   Pivot tables are a powerful data analysis tool that provide multidimensional summarization of data. 
-   They can be thought of as a **multidimensional version** of GroupBy aggregation.
...
`df.pivot_table('aggregate on which column?', `
                `index=by row index, columns=by column undex)`

Default: `aggfunc='mean'`, `df` can be any DataFrame

```{python}
#| echo: true
survival_by_class = titanic.pivot_table('survived', 
                                        index='sex', 
                                        columns='class')
...
## Titanic Dataset - Age group
`pd.cut(on which column , cutting point list)`
```{python}
#| echo: true
...
[Offset](https://pandas.pydata.org/docs/user_guide/timeseries.html#timeseries-offset-aliases)
...
`pd.date_range(start date, periods = #, freq='Offset Codes')`
...
Mean for each year `df.resample(any resample freq).mean()`, `df` can be any DataFrames

```{python}
#| echo: true
year_m = MSFT.resample('Y').mean()  #YE
print(year_m)
```

## Time Series - Resampling

Mean for each Quarter/bussiness day `df.resample(any resample freq).mean()`, `df` can be any DataFrames

```{python}
#| echo: true
qua_m = MSFT.resample('BQ').mean()  #BQE
print(qua_m)
```

## Time Series - Resampling

Value at end of each year `df.asfreq(any freq)`, `df` can be any DataFrames

```{python}
#| echo: true
year_v = MSFT.asfreq('Y')  
print(year_v)
year_c = MSFT.resample('Y').count()  
print(year_c)
```

## Time Series - Time-shifting
...
Shift data values `df.shift(#)`, default is day, `df` can be any DataFrames
...
weekly Rolling mean `df.rolling(#)` default day, `df` can be any DataFrames
