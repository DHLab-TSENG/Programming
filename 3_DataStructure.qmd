...
-   使用`[`index`]`取值
-   可用slice功能(`start`,`stop`,`step`)
    -   `1:`為從1開始到最後，`:2`為從0開始到2（不包括2）
    -   `::2`每隔一個整數新增一個數字
    -   `:`全部
-   `-` index為從後方計算回來
-   2D: 一個維度一個`[]`，前row後column
    -   `array`\[`row`\]\[`column`\]
...
-   `np.concatenate(`想結合的numpy array list`)`
-   一維矩陣直接往後貼
...
-   `np.concatenate(`想結合的numpy array list`)` 新增row
-   `np.row_stack([np1,np2])` or `np.vstack()` by row
-   `np.column_stack([np1,np2])` or `np.hstack()` by column
