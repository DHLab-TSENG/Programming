...
- Matplotlib is a multi-platform data visualization library for Python, built on NumPy arrays and designed to work with the broader SciPy stack.
- Created by John Hunter in 2002, with version 0.1 released in 2003.
- Widely adopted in the scientific Python community, including by the Space Telescope Science Institute (Hubble Telescope team)[2].

---

## Key Features

- Works across many operating systems and supports dozens of graphics backends and output types.
- Large user and developer base, making it a powerful and ubiquitous tool for scientific visualization in Python.
- Despite newer visualization tools, Matplotlib remains a vital part of the data science stack[2].

---

## Importing Matplotlib

- Standard import shorthands:
...
- The `plt` interface is most commonly used[2].

---

## Setting Plot Styles

- Use `plt.style` to set the visual style of your plots:

```python
plt.style.use('classic')
```

- Stylesheets are supported as of Matplotlib version 1.5[2].

---

## How to View Your Plots
...
| Feature                | MATLAB-style (`plt`)                  | Object-Oriented (`fig`, `ax`)        |
|------------------------|---------------------------------------|--------------------------------------|
| Syntax                 | Similar to MATLAB, stateful           | Explicit, uses Figure/Axes objects   |
| Example                | `plt.plot(x, y)`                      | `ax.plot(x, y)`                      |
| Panels/Subplots        | `plt.subplot(2, 1, 1)`                | `fig, ax = plt.subplots(2)`          |
| Best for               | Quick, simple plots                   | Complex, multi-panel, customized     |
...
Choose between MATLAB-style and object-oriented interfaces depending on your needs.
...
- In Matplotlib, a **figure** is the overall window or page, and **axes** are the plotting area.
...
- `ax` is an Axes instance (the actual plot area)[2].
...
- Alternatively, use the state-based interface:
...
## Object-Oriented Interface: Setting Properties

- Most `plt` functions have `ax` method equivalents:
...
- Set multiple properties at once with `ax.set()`:
...
Both state-based (plt) and object-oriented (ax) interfaces are available for flexibility.
...
- Useful for visualizing the relationship between two or more variables[2][1].
...
```
- The third argument (`'o'`) sets the marker shape. There are many marker options, such as `'o'`, `'.'`, `','`, `'x'`, `'+'`, `'v'`, `'^'`, `''`, `'s'`, `'d'`, etc.
...
```
- Here, x/y positions, size, and color all encode different data dimensions[2][1].
...
- `plt.scatter` is less efficient for large datasets since it allows per-point customization and must render each point individually[2][1].

---

## Summary

- Use `plt.plot` for simple scatter plots or large datasets.
- Use `plt.scatter` for advanced scatter plots with variable size, color, and multidimensional data visualization.
- Both methods offer a range of marker styles and customization options[2][1].
...
- For example, reporting a measurement as $$74 \pm 5$$ is much more informative than just 74[1].
...
- `fmt` controls the marker and line style (same as in `plt.plot`)[12][2].
...
- `capsize`: size of the error bar caps[12][2].
...
```
- You can combine `xerr` and `yerr` for both directions[12][6].
...
plt.show()
```
- This approach visually communicates where the model is more or less certain[12].
...
- For continuous error visualization, also explore `plt.fill_between`[12][2].
...
- For continuous error regions, use `plt.fill_between` for a more intuitive visualization[12].
...
- Matplotlib provides three main functions for this purpose:
  ...
  - `plt.imshow` for displaying images[1]
...
```
- `RdGy` is a good colormap for centered data. Matplotlib offers many colormaps (see `plt.cm.` in IPython for options)[1].
...
- Adjust the aspect ratio with `plt.axis(aspect='image')` to ensure equal scaling[1].
...
- For 3D contour plotting, see the "Three-dimensional Plotting in Matplotlib" section.
- Refer to the Matplotlib documentation for more customization options[1].
...
print(counts)
```
- Output: `[ 12 190 468 301 29]` (example)[2][1]
...
- For higher dimensions, use `np.histogramdd`[2][1].
...
ax.legend()
```
- By default, all labeled elements are included in the legend[2][1].
...
plt.colorbar()
```
- This displays the image and adds a colorbar indicating the mapping from data values to colors[2][1].
...
> **Note:** The `jet` colormap (old default) is not recommended for quantitative data because it does not provide a uniform progression in brightness, which can mislead interpretation[2][1].
...
```
- `ha` stands for *horizontal alignment* and can be `'left'`, `'center'`, or `'right'`[12][1].
...
## Understanding Ticks in Matplotlib

- **Ticks** are the markers on the axes that denote specific data points.
- Each axis in Matplotlib (x and y) has both **major** and **minor** ticks:
  - *Major ticks* are more prominent and usually labeled.
  - *Minor ticks* are smaller and often unlabeled by default.
- Ticks and their labels can be customized for better clarity and presentation[13][15].

---

## Accessing and Customizing Ticks

- Each `Axes` object contains `xaxis` and `yaxis` attributes, which in turn have properties for managing ticks, lines, and labels.
- You can control tick locations and formatting using **locators** and **formatters**.

---

## Hiding Ticks or Labels

- Use `NullLocator` and `NullFormatter` to hide ticks or labels:
...
# Hide all x-axis labels (but keep ticks/gridlines)
ax.xaxis.set_major_formatter(plt.NullFormatter())
```

- Useful for image grids or when ticks are not informative[13].

---

## Controlling the Number of Ticks

- Subplots can become crowded with default ticks.
...
```
- For regular spacing, use `MultipleLocator`:

```python
ax.xaxis.set_major_locator(plt.MultipleLocator(np.pi / 2))
ax.xaxis.set_minor_locator(plt.MultipleLocator(np.pi / 4))
```
- This is especially useful for mathematical plots (e.g., multiples of π)[13].

---

## Custom Tick Formats

- For custom tick labels, use `FuncFormatter` with your own formatting function:
...
```
- This allows for LaTeX-style math labels and highly customized tick text[13][15].

---

## Summary of Common Locators and Formatters

| Locator Class         | Description                                      |
|----------------------|--------------------------------------------------|
| `NullLocator`        | No ticks                                         |
| `FixedLocator`       | Fixed tick locations                             |
| `IndexLocator`       | For index plots (e.g., x = range(len(y)))        |
| `LinearLocator`      | Evenly spaced ticks                              |
| `LogLocator`         | Logarithmic ticks                                |
| `MultipleLocator`    | Ticks at multiples of a base                     |
| `MaxNLocator`        | Up to a maximum number of ticks at nice locations|
| `AutoLocator`        | (Default) auto ticks                             |
| `AutoMinorLocator`   | Auto minor ticks                                 |
...
| `FormatStrFormatter` | Format string for each value                     |
| `ScalarFormatter`    | (Default) scalar values                          |
| `LogFormatter`       | For log axes                                     |

---

## Practical Tips

- Use `MaxNLocator` to avoid overcrowded ticks, especially in subplots.
- Use `MultipleLocator` and `FuncFormatter` for mathematical or custom tick intervals and labels.
- Hide ticks or labels with `NullLocator` and `NullFormatter` when they do not add value.
- For advanced formatting, leverage LaTeX support in tick labels for mathematical notation[13][15].
...
- Customizing styles allows you to create more attractive, readable, or publication-ready plots that match your preferences or requirements[9][13].
...
- Stylesheets are `.mplstyle` files, similar to `.matplotlibrc` files, but easier to use and share[9][12][13].
...
```
- Later styles in the list override earlier ones if they set the same properties[12][15].
...
- While Basemap is somewhat dated and can be slow for complex maps, it remains a powerful tool for many geographic visualization tasks in Python[1][12].
...
```
- For some backgrounds, you may also need the `pillow` package (for image handling)[1][12].
...
plt.show()
```
- Each projection requires specific parameters (e.g., center latitude/longitude, width/height)[1].
...
plt.show()
```
- This map shows population (color) and area (size) of cities, with geographic context[9][1].
...
plt.show()
```
- This example uses a divergent colormap to show positive and negative temperature anomalies[1].
...
- **Legends and colorbars** are essential for interpreting size, color, and other encodings on your map[1][9].
...
- Seaborn makes statistical visualization easier, more concise, and more visually appealing[1].
