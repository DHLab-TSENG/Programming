...
```{python}
#| echo: true
df_air['concentration'] = pd.to_numeric(df_air['concentration'].replace("x",np.nan))
```
...
```{python}
plt.plot(pm25Linkou2024['DateTime'], pm25Linkou2024['concentration'],label="林口PM2.5")
plt.legend()
plt.show()
```
...
sns.lineplot(x=pm25Linkou2024['DateTime'], y=pm25Linkou2024['concentration'])
```
...
plt.scatter(air_wide["concentration","NO2"],air_wide["concentration","SO2"], alpha=0.2,
            s = 100*air_wide["concentration","WIND_SPEED"],marker='o')
plt.show()
```
